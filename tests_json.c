...
static int has_value(enum kvp_json_type type)
{
...
static int test(const char* name, int stream, char sep, struct expect* seq, size_t seqlen, const char* buf, size_t len)
{
    int success = 1;
    ...
    if(success) {
        ...
        printf(C_RED("FAIL") " %s: "
                             "expect " C_BOLD("%s") " %s / "
                                                    "actual " C_BOLD("%s") " %s\n",
            name, json_typename[expect], expect_str, json_typename[actual], actual_str);
    }
    kvp_close(&json2);
    return success;
}
...
int main()
{
    ...
    if(success) {
        ...
        printf(C_RED("FAIL") " %s: "
                             "expect " C_BOLD("%s") " %s / "
                                                    "actual " C_BOLD("%s") " %s\n",
            "first test", json_typename[expect], expect_str, json_typename[actual], actual_str);
    }
    kvp_close(&json);
}
